...
import { useMetrics, useProducts } from "@/lib/use-data"
...
export default function RevenuePage() {
  ...
  const { data: allMetrics, loading } = useMetrics()
    ...
    <div className="space-y-6">
      ...
      {!loading && data.length === 0 && (
        <Card className="p-12 bg-white border border-gray-100 shadow-none text-center">
          ...
          <h2 className="text-lg font-bold text-gray-800 mb-1">No revenue data</h2>
          <p className="text-sm text-gray-500 mb-6">Import transaction data to see revenue analytics.</p>
          <Link href="/import">
        ...
        <Tabs defaultValue="revenue" className="space-y-4">
                ...
                <TableBody>
                  ...
                  {data.length > 0 && (
                    <TableRow className="bg-gray-50 font-bold">
                      ...
                      <TableCell className="text-right">{(() => { const total = data.reduce((s, d) => s + d.revenue, 0); return total > 0 ? formatPercent(data.reduce((s, d) => s + d.annualRevenue, 0) / total) : '—' })()}</TableCell>
          ...
          <TabsContent value="plans" className="space-y-4">
            <Card className="bg-white border border-gray-100 shadow-none overflow-hidden">
              ...
              {products.length === 0 ? (
                <div className="p-8 text-center text-sm text-gray-400">No products found. Products are created from seed data or matched during import.</div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow className="border-b-2 border-black">
                  ...
                  <TableBody>
                    {[...products].sort((a, b) => b.revenue - a.revenue).map(p => (
                      <TableRow key={p.id}>
                        ...
                        <TableCell className="text-sm text-right">
                          {totalProductRevenue > 0 ? formatPercent(p.revenue / totalProductRevenue) : '—'}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
